...
from modules import RNNEncoder, SimpleSoftmaxLayer, BasicAttn, BidirectionalAttn, CharacterLevelCNN, CoAttn, CrossAttn
...
class QAModel(object):
    ...
    def __init__(self, FLAGS, id2word, word2id, emb_matrix, char2id, id2char, char_vocab_size):
        """
        ...
        Inputs:
          ...
          id2char: dictionary mapping char idx (int) to character
          char2id: dictionary mapping character to char idx (int)
    ...
    def add_embedding_layer(self, emb_matrix):
        ...
        with vs.variable_scope("embeddings"):
            ...
            if self.FLAGS.char_embedding:
                char_embedding_matrix = tf.get_variable(name = "char_embed_matrix", 
                                                        shape = [self.char_vocab_size, self.FLAGS.char_embedding_size], 
                                                        dtype=tf.float32) # shape (char_vocab_size, char_embedding_size)
                
                self.context_char_embs = embedding_ops.embedding_lookup(char_embedding_matrix, self.context_char_ids) # shape (batch_size, context_len, word_len, char_embedding_size)
                self.qn_char_embs = embedding_ops.embedding_lookup(char_embedding_matrix, self.qn_char_ids) # shape (batch_size, question_len, word_len, char_embedding_size)
                # concatenate to context embedgings
    ...
    def build_graph(self):
        ...
        if self.FLAGS.char_embedding:
            ...
            charcnn = CharacterLevelCNN(self.keep_prob, self.FLAGS.char_embedding_size)
            self.cnn_context_char_embs = charcnn.build_graph(self.context_char_embs) # shape (batch_size, context_len, filter_size)
            self.cnn_qn_char_embs = charcnn.build_graph(self.qn_char_embs)           # shape (batch_size, question_len, filter_size)
            self.context_embs = tf.concat([self.context_embs, self.cnn_context_char_embs], axis =2)
            self.qn_embs = tf.concat([self.qn_embs, self.cnn_qn_char_embs], axis =2)
        ...
        # model_type can be baseline/bidaf/coatten/hybrid
        if self.FLAGS.model_type.strip()=="bidaf":
            print("Creating Bidirectional Attention Graph...")
            # Use context hidden states to attend to question hidden states
            biattn_layer = BidirectionalAttn(self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2)
            att_vec, c_prime = biattn_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens, self.context_mask) # attn_output is shape (batch_size, context_len, hidden_size*2)

            # # Concat attn_output to context_hiddens to get blended_reps
            blended_reps = tf.concat([context_hiddens, att_vec, tf.multiply(context_hiddens,att_vec), tf.multiply(context_hiddens,c_prime)], axis=2) # (batch_size, context_len, hidden_size*4)

            # # Bi-DAF Modeling Layer (Full BiDAF)
            # with tf.variable_scope('bidaf_modelinglayer'):
            #     rnn_cell_fw = rnn_cell.GRUCell(self.FLAGS.hidden_size)
            #     rnn_cell_fw = rnn_cell.DropoutWrapper(rnn_cell_fw, input_keep_prob=self.keep_prob)
            #     rnn_cell_bw = rnn_cell.GRUCell(self.FLAGS.hidden_size)
            #     rnn_cell_bw = rnn_cell.DropoutWrapper(rnn_cell_bw, input_keep_prob=self.keep_prob)
            #     # Note: fw_out and bw_out are the hidden states for every timestep.
            #     # Each is shape (batch_size, seq_len, hidden_size).
            #     (fw_out, bw_out), _ = tf.nn.bidirectional_dynamic_rnn(rnn_cell_fw, rnn_cell_bw, inputs=blended_reps, dtype=tf.float32)
            #     # Concatenate the forward and backward hidden states
            #     blended_reps = tf.concat([fw_out, bw_out], 2)
            #     # Apply dropout
            #     blended_reps = tf.nn.dropout(blended_reps, self.keep_prob)

        elif self.FLAGS.model_type.strip()=="coatten":
            print("Creating Co-Attention Graph...")
            # Compute Co-Attention Layer
        ...
        elif self.FLAGS.model_type.strip()=="hybrid":
            print("Creating Hybrid Graph (Blended Approach)...")
            # Use context hidden states to attend to question hidden states
            biattn_layer = BidirectionalAttn(self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2)
            att_vec, c_prime = biattn_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens, self.context_mask) # attn_output is shape (batch_size, context_len, hidden_size*2)
            ...
            # Concat attn_output to context_hiddens to get blended_reps
            blended_reps = tf.concat([context_hiddens, att_vec, attn_output, tf.multiply(context_hiddens,att_vec), tf.multiply(context_hiddens,c_prime)], axis=2) # (batch_size, context_len, hidden_size*4)

            # # Pass through another Modeling Layer
            # with tf.variable_scope('hybrid_modelinglayer'):
            #     rnn_cell_fw = rnn_cell.GRUCell(self.FLAGS.hidden_size)
            #     rnn_cell_fw = rnn_cell.DropoutWrapper(rnn_cell_fw, input_keep_prob=self.keep_prob)
            #     rnn_cell_bw = rnn_cell.GRUCell(self.FLAGS.hidden_size)
            #     rnn_cell_bw = rnn_cell.DropoutWrapper(rnn_cell_bw, input_keep_prob=self.keep_prob)
        ...
        # Not Implemented
        # elif self.FLAGS.model_type.strip()=="rnet":
        #     print("Creating RNET Graph")
        #     # Use context hidden states to attend to question hidden states
        #     attn_layer = SelfAttnRnet(self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2)
        #     _, attn_output = attn_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens) # attn_output is shape (batch_size, context_len, hidden_size*2)

        #     # Concat attn_output to context_hiddens to get blended_reps
        #     blended_reps = tf.concat([context_hiddens, attn_output], axis=2) # (batch_size, context_len, hidden_size*4)

        elif self.FLAGS.model_type.strip()=="recur":
            print("Creating Recursive Cross Attention Graph")
            # Use context hidden states to attend to question hidden states
            attn_layer = CrossAttn(self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2)
            output_context1, output_qn1 = attn_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens, self.context_mask) # attn_output is shape (batch_size, context_len, hidden_size*2)
            output_context2, output_qn2 = attn_layer.build_graph(output_qn1, self.qn_mask, output_context1, self.context_mask) # attn_output is shape (batch_size, context_len, hidden_size*2)

            # Concat attn_output to context_hiddens to get blended_reps
            blended_reps = tf.concat([context_hiddens, output_context1, output_context2], axis=2) # (batch_size, context_len, hidden_size*6)
            with tf.variable_scope('recur2'):
                u_lstm_forward = tf.contrib.rnn.BasicLSTMCell(self.FLAGS.hidden_size*2) 
                u_lstm_backward = tf.contrib.rnn.BasicLSTMCell(self.FLAGS.hidden_size*2)
                (fw_out, bw_out),_ = tf.nn.bidirectional_dynamic_rnn(cell_bw=u_lstm_backward,cell_fw=u_lstm_forward,dtype=tf.float32,inputs=blended_reps,time_major=False)
                blended_reps = tf.concat([fw_out, bw_out], 2)
                blended_reps = tf.nn.dropout(blended_reps, self.keep_prob)

        elif self.FLAGS.model_type.strip()=="baseline":
            ...
            raise Exception("Unexpected value of FLAGS.model_type: %s" % self.FLAGS.model_type)
    ...
    def get_dev_loss(self, session, dev_context_path, dev_qn_path, dev_ans_path):
        ...
        for batch in get_batch_generator(self.word2id, self.char2id, dev_context_path, dev_qn_path, dev_ans_path, self.FLAGS.batch_size, context_len=self.FLAGS.context_len, question_len=self.FLAGS.question_len, word_len=self.FLAGS.word_len, char_embedding = self.FLAGS.char_embedding, discard_long=True):
    ...
    def check_f1_em(self, session, context_path, qn_path, ans_path, dataset, num_samples=100, print_to_screen=False):
        ...
        for batch in get_batch_generator(self.word2id, self.char2id, context_path, qn_path, ans_path, self.FLAGS.batch_size, context_len=self.FLAGS.context_len, question_len=self.FLAGS.question_len, word_len=self.FLAGS.word_len, char_embedding = self.FLAGS.char_embedding, discard_long=False):
    ...
    def train(self, session, train_context_path, train_qn_path, train_ans_path, dev_qn_path, dev_context_path, dev_ans_path):
        ...
        while self.FLAGS.num_epochs == 0 or epoch < self.FLAGS.num_epochs:
            ...
            # Loop over batches
            for batch in get_batch_generator(self.word2id, self.char2id, train_context_path, train_qn_path, train_ans_path, self.FLAGS.batch_size, context_len=self.FLAGS.context_len, question_len=self.FLAGS.question_len, word_len=self.FLAGS.word_len, char_embedding = self.FLAGS.char_embedding, discard_long=True):

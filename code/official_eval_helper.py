...
from data_batcher import padded, word_to_char_ids, padded_words, Batch
...
def refill_batches(batches, word2id, char2id, qn_uuid_data, context_token_data, qn_token_data, batch_size, context_len, question_len, word_len, char_embedding):
    """
...
def get_batch_generator(word2id, char2id, qn_uuid_data, context_token_data, qn_token_data, batch_size, context_len, question_len, word_len, char_embedding):
    """
    ...
    while True:
        if len(batches) == 0:
            refill_batches(batches, word2id, char2id, qn_uuid_data, context_token_data, qn_token_data, batch_size, context_len, question_len, word_len, char_embedding)
        if len(batches) == 0:
            break
...
def generate_answers(session, model, word2id, char2id, qn_uuid_data, context_token_data, qn_token_data):
    ...
    for batch in get_batch_generator(word2id, char2id, qn_uuid_data, context_token_data, qn_token_data, model.FLAGS.batch_size, model.FLAGS.context_len, model.FLAGS.question_len, model.FLAGS.word_len, model.FLAGS.char_embedding):

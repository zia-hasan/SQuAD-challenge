...
class Batch(object):
    ...
    def __init__(self, context_ids, context_mask, context_tokens, qn_ids, qn_mask, qn_tokens, ans_span, ans_tokens, context_char_ids=None, qn_char_ids=None, uuids=None):
        """
        Inputs:
...
def word_to_char_ids(word, char2id, word_len):
    """Turns a word string into char indices
    e.g. "hello" -> [7, 5, 13, 24, 20] (not the real ids/just for explanation)
    Note any token that isn't in the char2id mapping gets mapped to the id for <UNK>
    """
    tokens = list(word) # list of strings
    ids = [char2id.get(c, UNK_ID) for c in tokens]
    if len(ids) > word_len: # truncate needed 
        ids = ids[:word_len] 
    else:  # padding needed
...
def padded_words(token_batch, char2id, word_len, batch_pad=0):
    """
    Inputs:
      token_batch: List (length batch size) of lists of ints.
    ...
    """
    maxlen = max(map(lambda x: len(x), token_batch)) if batch_pad == 0 else batch_pad
    return map(lambda token_list: token_list + [[PAD_ID]*word_len] * (maxlen - len(token_list)), token_batch)

def refill_batches(batches, word2id, char2id, context_file, qn_file, ans_file, batch_size, context_len, question_len, word_len, char_embedding, discard_long):
    """
    ...
    Inputs:
      ...
      context_len, question_len, word_len: max length of context, question and word respectively
      char_embedding: flag to enable character level embeddings or not
    ...
    while context_line and qn_line and ans_line: # while you haven't reached the end
        ...
        if char_embedding:
            for word in split_by_whitespace(context_line):
                _, char_ids = word_to_char_ids(word, char2id, word_len)
                context_char_ids.append(char_ids)
                if(len(context_char_ids)==context_len): break
            for word in split_by_whitespace(qn_line):
                _, char_ids = word_to_char_ids(word, char2id, word_len)
                qn_char_ids.append(char_ids)
                if(len(qn_char_ids)==question_len): break
...
def get_batch_generator(word2id, char2id, context_path, qn_path, ans_path, batch_size, context_len, question_len, word_len, char_embedding, discard_long):
    """
    ...
    while True:
        if len(batches) == 0: # add more batches
            refill_batches(batches, word2id, char2id, context_file, qn_file, ans_file, batch_size, context_len, question_len, word_len, char_embedding, discard_long)
        if len(batches) == 0:
            break

        # Get next batch. These are all lists length batch_size
        (context_ids, context_tokens, qn_ids, qn_tokens, ans_span, ans_tokens, context_char_ids, qn_char_ids) = batches.pop(0)
        ...
        if char_embedding:
            ...
            context_char_ids = padded_words(context_char_ids, char2id, word_len, context_len)
            context_char_ids = np.array(context_char_ids)

            qn_char_ids = padded_words(qn_char_ids, char2id, word_len, question_len)
        ...
        batch = Batch(context_ids, context_mask, context_tokens, qn_ids, qn_mask, qn_tokens, ans_span, ans_tokens, context_char_ids, qn_char_ids)

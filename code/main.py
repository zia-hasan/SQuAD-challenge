...
from create_char_dict import get_char_dict
...
tf.app.flags.DEFINE_integer("minitrain", 0, "Use mini_train dataset for fast checking whether we can quickly overfit or not (sanity check)")
tf.app.flags.DEFINE_integer("num_epochs", 0, "Number of epochs to train. 0 means train indefinitely")
tf.app.flags.DEFINE_integer("char_embedding", 1, "Whether char-level CNN embedding is on or not. 0 means off")
tf.app.flags.DEFINE_string("model_type", "baseline", "Which model to use: baseline/bidaf/coatten/hybrid/recur")
...
tf.app.flags.DEFINE_integer("word_len", 15, "The maximum word length of your model")
tf.app.flags.DEFINE_integer("char_embedding_size", 20, "Size of the char embedding size")
...
def main(unused_argv):
    ...
    if FLAGS.minitrain:
        minitrain_context_path = os.path.join(FLAGS.data_dir, "minitrain.context")
        minitrain_qn_path = os.path.join(FLAGS.data_dir, "minitrain.question")
        minitrain_ans_path = os.path.join(FLAGS.data_dir, "minitrain.span")
        print("Generating Mini Training Set (first run only)...")
        def reduce(inputfile, outputfile, size):
            ...
            ifile.close()
            ofile.close()
        reduce(train_context_path, minitrain_context_path, size = EXTRACT_NUM_LINES)
        reduce(train_qn_path, minitrain_qn_path, size = EXTRACT_NUM_LINES)
        reduce(train_ans_path, minitrain_ans_path, size = EXTRACT_NUM_LINES)
        train_context_path = os.path.join(FLAGS.data_dir, "minitrain.context")
        train_qn_path = os.path.join(FLAGS.data_dir, "minitrain.question")
        train_ans_path = os.path.join(FLAGS.data_dir, "minitrain.span")
    ...
    if not os.path.isfile(char_dict_pickle_file):
        print("Generating Frequenct Character Dictionary (first run only)...")
        get_char_dict(os.path.join(FLAGS.data_dir, "train.context"), os.path.join(FLAGS.data_dir, "train.question"), dev_context_path, dev_qn_path)
    else:
        print("Loading Frequenct Character Dictionary from Disk...")
    # char2id, id2char, char_vocab_size , _ = get_char_dict(train_context_path, train_qn_path, dev_context_path, dev_qn_path)
    char2id, id2char, char_vocab_size = pickle.load(open(char_dict_pickle_file, "rb" ))

    # Initialize model
    qa_model = QAModel(FLAGS, id2word, word2id, emb_matrix, char2id, id2char, char_vocab_size)
